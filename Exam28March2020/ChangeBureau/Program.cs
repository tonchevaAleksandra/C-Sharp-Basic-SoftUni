using System;

namespace ChangeBureau
{
    class Program
    {
        static void Main(string[] args)
        {
            //  На първия ред – броят биткойни. Цяло число в интервала[0…20]
            // На втория ред – броят китайски юана.Реално число в интервала[0.00… 50 000.00]
            // На третия ред – комисионната.Реално число в интервала[0.00... 5.00]
            int countBitcoins = int.Parse(Console.ReadLine());
            double countChM = double.Parse(Console.ReadLine());
            double commission = double.Parse(Console.ReadLine());
            //            1 биткойн = 1168 лева.
            // 1 китайски юан = 0.15 долара.
            // 1 долар = 1.76 лева.
            // 1 евро = 1.95 лева.
            //Обменното бюро има комисионна от 0 до 5 процента от крайната сума в евро.
            double bitcoinInBGN = countBitcoins * 1168;
            double chMtoDoll = countChM * 0.15;
            double dollarToBGN = chMtoDoll * 1.76;
            double totalsum = bitcoinInBGN + dollarToBGN;
            double euro = totalsum/ 1.95;
            double commissionInEuro = (euro * commission) / 100.0;
            double total = euro - commissionInEuro;
            Console.WriteLine($"{total:f2}");

        }
    }
}
